...
export class ClientReportService {
  ...
  showClientReportbyLocation(location_id:any){
    let headers = new HttpHeaders({'Authorization': 'Bearer'+this.authService.token});
    let URL = url_servicios+"/client_report/bylocation/"+location_id;
    return this.http.get(URL,{headers:headers});
  }
  getAllClientReportByLocation(
    ...
    let LINK = "";
     ...
     if(date_start){
      ...
      if(noteType)
        LINK+=`&noteType=${noteType}`
    let URL = url_servicios+'/client_report/bylocation/'+location_id+'/?page='+page+LINK;
    return this.http.get(URL, {headers:headers});
  }
